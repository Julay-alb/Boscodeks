...
// Viewport (contenedor de todos los toasts)
...
// Variantes de estilo para los toasts
const toastVariants = cva(
  'group relative pointer-events-auto flex w-full items-center justify-between space-x-4 overflow-hidden rounded-md border p-6 pr-8 shadow-lg transition-all ' +
    ...
    'data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=cancel]:translate-x-0',
  {
    variants: {
      variant: {
...
const Toast = React.forwardRef(({ className, variant, ...props }, ref) => {
  // Evitamos pasar props no válidas al DOM (como "dismiss")
  ...
  <ToastPrimitives.Action
    ...
    className={cn(
      'inline-flex h-8 shrink-0 items-center justify-center rounded-md border bg-transparent px-3 text-sm font-medium ' +
        ...
        'group-[.destructive]:border-destructive/30 group-[.destructive]:hover:bg-destructive group-[.destructive]:text-destructive-foreground',
      className
    )}
    {...props}
  />
  ...
  <ToastPrimitives.Close
    ...
    className={cn(
      'absolute right-2 top-2 rounded-md p-1 text-foreground/60 opacity-0 transition-opacity ' +
        'hover:text-foreground focus:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring group-hover:opacity-100 ' +
