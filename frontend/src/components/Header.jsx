...
import { Ticket, Plus, LogOut, Users } from 'lucide-react';
import { Button } from '@/components/ui/button';
import auth from '@/lib/auth';

const Header = ({ onNewTicket, onManageUsers, user }) => {
  return (
    <motion.header
      ...
      <div className="container mx-auto px-4 py-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            {user ? (
              <>
                <div className="text-sm text-purple-100 mr-2">{user.full_name || user.name}</div>
                ...
                {/* Botón para crear ticket (visible a todos los logueados) */}
                <Button
                  onClick={onNewTicket}
              ...
              <div className="text-purple-200 italic">Inicia sesión para crear tickets</div>
            )}
          </div>
        </div>
      </div>
